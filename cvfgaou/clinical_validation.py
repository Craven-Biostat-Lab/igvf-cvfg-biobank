...
def generate_variant_counts(input_functional_dataset, phenotype_df = None, test_intervals=None, output_bucket=None):
    """
    ...
    expected_columns_from_functional_dataset = ['chromosome', 'hg38_start', 'ref_allele', 'alt_allele', 'auth_reported_func_class']
    ...
    #infer columns
    input_functional_dataset['locus'] = 'chr'+input_functional_dataset['chromosome'].astype(str)+":"+input_functional_dataset['hg38_start'].astype(str)
    input_functional_dataset['alleles'] = input_functional_dataset.apply(lambda x: [x['ref_allele'], x['alt_allele']], axis = 1)
    ...
    hail_functional_df = hail_functional_df.annotate(locus=hl.parse_locus(hail_functional_df.locus))
    hail_functional_df = hail_functional_df.key_by('locus', 'alleles')
    
    mt_filtered = mt_filtered.filter_rows(hl.is_defined(hail_functional_df[mt_filtered.row_key]))
    ...
    if phenotype_df is not None:
        ...
        input_functional_dataset['chromosome'] = 'chr'+input_functional_dataset['chromosome']
        persons_with_findlay_score = get_cols_with_variants(input_functional_dataset, split_mt, contig_col = 'chromosome', pos_col = "hg38_start", ref_col = "ref_allele", alt_col = "alt_allele")
        
        # 3. Process phenotype DataFrame
        df_persons_conditions = phenotype_df.copy()

        phenotype_df = phenotype_df[phenotype_df['s'].isin(persons_with_findlay_score['s'])]
        ...
        mt_select = mt_Findlay_with_pheno.select_rows('functional_metadata')
...
def plot_fig_1(file_name):
    ...
    df_plot_filtered['functional_metadata'] = df_plot_filtered['functional_metadata'].apply(json.loads)
    df_plot_filtered['auth_reported_func_class'] = df_plot_filtered['functional_metadata'].apply(lambda x:x['auth_reported_func_class'])
...
def plot_fig_2(file_name):
    ...
    df_plot[['allele1', 'allele2']] = df_plot['alleles'].str.extract(r'\["([^"]+)"\s*,\s*"([^"]+)"\]')
    df_plot['variant'] = df_plot['locus']+':'+df_plot['allele1']+'_'+df_plot['allele2']
    
    df_fig2_filtered = df_plot[~df_plot['GT'].isin(['0/0', '0|0'])] # 78601 rows
    df_fig2_filtered['phenotype_metadata'] = df_fig2_filtered['phenotype_metadata'].apply(json.loads)
    df_fig2_filtered['age_at_diagnosis'] = df_fig2_filtered['phenotype_metadata'].apply(lambda x:x['age_at_diagnosis'])
    df_fig2_filtered['breast_or_ovarian_diagnosis'] = df_fig2_filtered['phenotype_metadata'].apply(lambda x:x['breast_or_ovarian_diagnosis'])
    df_fig2_filtered['sex_at_birth'] = df_fig2_filtered['phenotype_metadata'].apply(lambda x:x['sex_at_birth'])

    df_fig2_filtered['findlay_metadata'] = df_fig2_filtered['findlay_metadata'].apply(load_strings)
    df_fig2_filtered['auth_reported_func_class'] = df_fig2_filtered['findlay_metadata'].apply(lambda x:x['auth_reported_func_class'] if x else None)
    df_fig2_filtered['auth_reported_score'] = df_fig2_filtered['findlay_metadata'].apply(lambda x:x['auth_reported_score'] if x else None)
    df_fig2_filtered['clinvar_class'] = df_fig2_filtered['findlay_metadata'].apply(lambda x:x['clinvar_class'] if x else None)


    df_fig2_plot = df_fig2_filtered[['s', 'age_at_diagnosis', 'sex_at_birth', 'variant','auth_reported_func_class', 'auth_reported_score', 'clinvar_class', 'breast_or_ovarian_diagnosis']]

    filtered_df_plot_fig2 = df_fig2_plot.groupby('s').apply(select_variant).reset_index(drop=True)

    df_plot_females = filtered_df_plot_fig2[filtered_df_plot_fig2['sex_at_birth'] == 'Female']
    df_plot_breast_cancer = df_plot_females[['s', 'age_at_diagnosis', 'breast_or_ovarian_diagnosis', 'auth_reported_func_class']].drop_duplicates()
    df_plot_breast_cancer['breast_or_ovarian_diagnosis'] = df_plot_breast_cancer['breast_or_ovarian_diagnosis'].astype(bool)
    
    # LOF vs. unscored
    km_res_func= km_risk_analysis(df_plot_breast_cancer, "FUNC", 'breast_or_ovarian_diagnosis')
    km_res_func
    km_res_lof= km_risk_analysis(df_plot_breast_cancer, "LOF", 'breast_or_ovarian_diagnosis')
    km_res_lof
...
def select_variant(val):
    ...
    if len(lof_row): return lof_row.iloc[[0]]
    
    # priority 2 - INT
    int_row = val[val["auth_reported_func_class"] == "INT"]
    if len(int_row): return int_row.iloc[[0]]
...
def set_diagnosis_flag(df, column_name, keywords_list, new_column):
    ...
    df[new_column] = np.where(df[column_name].str.contains(pattern, case=False, na=False), 1, 0)
...
def generate_phenotype_df(df_persons_conditions_control, df_persons_conditions_case):
    ...
    df_pheno['condition_start_datetime'] = df_pheno['condition_start_datetime'].dt.tz_localize(None)
    df_pheno['date_of_birth'] = df_pheno['date_of_birth'].dt.tz_localize(None)
    df_pheno['age_at_diagnosis'] = ((df_pheno['condition_start_datetime'] - df_pheno['date_of_birth']).dt.days/365).round().astype("Int64")
    df_pheno['s'] = df_pheno['s'].astype(str)
    df_pheno = df_pheno[['s', 'standard_concept_name', 'standard_concept_code', 'race', 'ethnicity', 'sex_at_birth', 'age_at_diagnosis']]
    df_pheno.head(5)
...
def kmf_test(df, group, event_column):
    ...
    f = KaplanMeierFitter()
    f.fit(
            durations=g1['age_at_diagnosis'],
            event_observed=g1[event_column]
        )
